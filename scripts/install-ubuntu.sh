...
installer_config="$project_dir/deploy/ubuntu/install.env"
...
usage() {
  echo "Usage:"
  echo "  $0 install --archive PATH --version VERSION [--allow-unsupported-os]"
  echo "  $0 rollback --version VERSION [--allow-unsupported-os]"
}
...
validate_host() {
  ...
  if [[ "${ID:-}" != "ubuntu" ]]; then
    echo "PocketPortal's host installer currently supports Ubuntu only." >&2
    exit 1
  fi

  if [[ " $POCKETPORTAL_SUPPORTED_UBUNTU_VERSIONS " != *" ${VERSION_ID:-unknown} "* ]]; then
    if [[ "$allow_unsupported_os" != true ]]; then
      echo "Ubuntu ${VERSION_ID:-unknown} is outside the tested support matrix." >&2
      echo "Use --allow-unsupported-os only after reviewing the risk." >&2
      exit 1
    fi
    echo "Warning: continuing on unsupported Ubuntu ${VERSION_ID:-unknown}."
  fi
...
if [[ ! -f "$config_file" ]]; then
  cp "$project_dir/deploy/ubuntu/pocketportal.properties" "$config_file"
fi
cp "$project_dir/deploy/ubuntu/pocketportal.service" "$service_file"
