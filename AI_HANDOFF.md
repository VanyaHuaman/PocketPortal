...
- Version `0.1.10-SNAPSHOT` is active on the real server. Earlier upgrade and rollback behavior was verified between `0.1.0` and `0.1.1`; subsequent dashboard, generalized-Linux, enriched-observation, screenshot, wake-action, limited ADB bridge, and adaptive device-frame releases were deployed through the same versioned upgrade path.
