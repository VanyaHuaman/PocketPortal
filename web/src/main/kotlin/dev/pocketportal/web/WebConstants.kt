...
object WebConstants {
    ...
    const val DEVICE_WAKE_PATH = "/api/devices/{serial}/actions/wake"
    const val DEVICE_ADB_BRIDGE_PATH = "/api/devices/{serial}/adb"
    const val DEVICE_SERIAL_PARAMETER = "serial"
    const val SCREENSHOT_UNAVAILABLE_CODE = "device_screenshot_unavailable"
    const val DEVICE_WAKE_FAILED_CODE = "device_wake_failed"
    const val AUTHORIZATION_HEADER = "Authorization"
    const val BEARER_PREFIX = "Bearer "
    const val ADB_BRIDGE_BUFFER_BYTES = 65_536
    const val ADB_BRIDGE_UNAUTHORIZED_REASON = "unauthorized"
    const val ADB_BRIDGE_UNAVAILABLE_REASON = "bridge unavailable"
